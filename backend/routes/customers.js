...
router.post('/', (req, res) => {
    ...
    const query = 'INSERT INTO customers (name, phone, email, address, type) VALUES (?, ?, ?, ?, ?)';
...
router.put('/:id', (req, res) => {
    ...
    const query = 'UPDATE customers SET name = ?, phone = ?, email = ?, address = ?, type = ?, total_purchases = ?, last_purchase = ? WHERE id = ?';
...
router.get('/stats', (req, res) => {
    db.get('SELECT COUNT(*) as total_customers, SUM(total_purchases) as total_revenue, AVG(total_purchases) as avg_purchase, COUNT(CASE WHEN type = "wholesale" THEN 1 END) as wholesale_count, COUNT(CASE WHEN type = "regular" THEN 1 END) as regular_count, COUNT(CASE WHEN type = "vip" THEN 1 END) as vip_count FROM customers', (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(row || {});
    });
});
