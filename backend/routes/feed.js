...
router.post('/', (req, res) => {
    ...
    if (!feed_type || quantity_kg === undefined) return res.status(400).json({ error: 'Missing required fields' });
    const query = 'INSERT INTO feed (feed_type, quantity_kg) VALUES (?, ?)';
...
router.put('/:id', (req, res) => {
    const { id } = req.params;
    const { feed_type, quantity_kg } = req.body;
    const query = 'UPDATE feed SET feed_type = ?, quantity_kg = ?, last_updated = CURRENT_TIMESTAMP WHERE id = ?';
...
router.post('/consumption', (req, res) => {
    ...
    if (!feed_type || !date || bags === undefined) return res.status(400).json({ error: 'Missing required fields' });
    const query = 'INSERT INTO feed_consumption (feed_type, date, bags, bag_size, total_kg, condition) VALUES (?, ?, ?, ?, ?, ?)';
...
router.post('/stock', (req, res) => {
    ...
    if (!feed_type || !date || bags === undefined) return res.status(400).json({ error: 'Missing required fields' });
    const query = 'INSERT INTO feed_stock_records (feed_type, date, bags, bag_size, total_kg, total_cost, supplier) VALUES (?, ?, ?, ?, ?, ?, ?)';
