...
router.post('/eggs', (req, res) => {
    ...
    if (!buyer_name || !sale_date) return res.status(400).json({ error: 'Missing required fields' });
    const query = 'INSERT INTO egg_sales (buyer_name, sale_date, egg_size, crates, pieces, price_per_crate, total_eggs, total, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
router.put('/eggs/:id', (req, res) => {
    const { id } = req.params;
    const { buyer_name, sale_date, egg_size, crates, pieces, price_per_crate, total_eggs, total, notes } = req.body;
    const query = 'UPDATE egg_sales SET buyer_name = ?, sale_date = ?, egg_size = ?, crates = ?, pieces = ?, price_per_crate = ?, total_eggs = ?, total = ?, notes = ? WHERE id = ?';
...
router.post('/chickens', (req, res) => {
    ...
    if (!buyer || !sale_date || !chicken_type || !quantity) return res.status(400).json({ error: 'Missing required fields' });
    const query = 'INSERT INTO chicken_sales (buyer, sale_date, chicken_type, quantity, price, status, total, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
...
router.put('/chickens/:id', (req, res) => {
    const { id } = req.params;
    const { buyer, sale_date, chicken_type, quantity, price, status, total, notes } = req.body;
    const query = 'UPDATE chicken_sales SET buyer = ?, sale_date = ?, chicken_type = ?, quantity = ?, price = ?, status = ?, total = ?, notes = ? WHERE id = ?';
...
router.get('/stats', (req, res) => {
    db.get('SELECT SUM(total) as egg_total, COUNT(*) as egg_count FROM egg_sales', (err, eggRow) => {
        ...
        db.get('SELECT SUM(total) as chicken_total, SUM(quantity) as chicken_count FROM chicken_sales', (err, chickenRow) => {
            if (err) return res.status(500).json({ error: err.message });
            db.get('SELECT SUM(total) as month_total FROM (SELECT total FROM egg_sales WHERE strftime("%Y-%m", sale_date) = strftime("%Y-%m", "now") UNION ALL SELECT total FROM chicken_sales WHERE strftime("%Y-%m", sale_date) = strftime("%Y-%m", "now"))', (err, monthRow) => {
                if (err) return res.status(500).json({ error: err.message });
                res.json({ eggRevenue: eggRow ? eggRow.egg_total || 0 : 0, eggCount: eggRow ? eggRow.egg_count || 0 : 0, chickenRevenue: chickenRow ? chickenRow.chicken_total || 0 : 0, chickenCount: chickenRow ? chickenRow.chicken_count || 0 : 0, totalRevenue: (eggRow ? eggRow.egg_total || 0 : 0) + (chickenRow ? chickenRow.chicken_total || 0 : 0), monthRevenue: monthRow ? monthRow.month_total || 0 : 0 });
            });
        });
    });
});
