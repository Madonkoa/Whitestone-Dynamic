// routes/sales.js - Sales CRUD (Eggs & Chickens)
...
// Get all egg sales
router.get('/eggs', verifyToken, checkPermission('sales_view'), (req, res) => {
...
// Record egg sale
router.post('/eggs', verifyToken, checkPermission('sales_crud'), (req, res) => {
...
// Update egg sale
router.put('/eggs/:id', verifyToken, checkPermission('sales_crud'), (req, res) => {
    const { id } = req.params;
    const { buyer_name, sale_date, egg_size, crates, pieces, price_per_crate, total_eggs, total, notes } = req.body;
...
// Delete egg sale
router.delete('/eggs/:id', verifyToken, checkPermission('sales_crud'), (req, res) => {
    const { id } = req.params;
...
// Get all chicken sales
router.get('/chickens', verifyToken, checkPermission('sales_view'), (req, res) => {
...
// Record chicken sale
router.post('/chickens', verifyToken, checkPermission('sales_crud'), (req, res) => {
    ...
    db.run(query, [buyer, sale_date, chicken_type, quantity, price || 0, status || 'undressed', total || 0, notes], function (err) {
        ...
        res.json({
            success: true,
            id: this.lastID,
            message: 'Chicken sale recorded successfully'
        });
    });
});

// Update chicken sale
router.put('/chickens/:id', verifyToken, checkPermission('sales_crud'), (req, res) => {
    const { id } = req.params;
    const { buyer, sale_date, chicken_type, quantity, price, status, total, notes } = req.body;
    ...
    db.run(query, [buyer, sale_date, chicken_type, quantity, price, status, total, notes, id], function (err) {
        ...
        res.json({
            success: true,
            message: 'Chicken sale updated successfully'
        });
    });
});

// Delete chicken sale
router.delete('/chickens/:id', verifyToken, checkPermission('sales_crud'), (req, res) => {
    const { id } = req.params;
    ...
    db.run('DELETE FROM chicken_sales WHERE id = ?', [id], function (err) {
        ...
        res.json({
            success: true,
            message: 'Chicken sale deleted successfully'
        });
    });
});

// Get sales stats
router.get('/stats', verifyToken, checkPermission('sales_view'), (req, res) => {
    // Total egg sales
